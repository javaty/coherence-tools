...
public class ParallelStep
    ...
    public void execute(ExecutionContext context)
        ...
        try
            ...
            if (!localSteps.isEmpty())
                {
                localExecutor = new Thread(new ParallelStepExecutor(context.getLocalExecutor(), context, localSteps));
                localExecutor.start();
                }
            if (!remoteSteps.isEmpty())
                {
                remoteExecutor = new Thread(new ParallelStepExecutor(context.getRemoteExecutor(), context, remoteSteps));
    ...
    private class ParallelStepExecutor implements Runnable
        {
        ...
        public void run()
            ...
            try
                ...
                List<Future<StepResult>> results = m_executor.invokeAll(callableSteps);
